 ...
 *
 */
public class SimpleCreditMarketSimulation implements
	 ...
	 * @param population
	 */
	public SimpleCreditMarketSimulation (){
		this.buyersId = new int[0];
		this.sellersId = new int[0];
	}
	
	public SimpleCreditMarketSimulation(SimulationController simulationController,MacroSimulation simulation, 
	...
	public void step() {
		if(transaction instanceof ConstrainedCreditMechanism) {
			ConstrainedCreditMechanism mechanism = (ConstrainedCreditMechanism) transaction;
			mechanism.setDemanderSupplierList(new HashMap<Long,ArrayList<MacroAgent>>());
			
			for (Agent buyer : population.getBuyers().getAgents()) {
				MacroAgent b = (MacroAgent)buyer;
				if(b.isActive(simulation.getActiveMarketId())){
					mechanism.getDemanderSupplierList().put(b.getAgentId(), new ArrayList<MacroAgent>());
				}
			}				
		}
		for(int i=0;i<this.nbRuns&&!this.mixer.closed(population, simulation);i++){
			// Check whether an agent has already requested all active sellers. If so set agent inactive.
			if(transaction instanceof ConstrainedCreditMechanism&&i>0) {
				ArrayList<MacroAgent> activeSellers = new ArrayList<MacroAgent>();
				
				for (Agent seller : population.getSellers().getAgents()) {
					MacroAgent s = (MacroAgent)seller;
					if(s.isActive(simulation.getActiveMarketId())){
						activeSellers.add(s);
					}
				}
				...
				for (Agent buyer : population.getBuyers().getAgents()) {
					...
					if(b.isActive(simulation.getActiveMarketId())){
						ConstrainedCreditMechanism mechanism = (ConstrainedCreditMechanism) transaction;
						ArrayList<MacroAgent> requestedSellers = mechanism.getDemanderSupplierList().get(b.getAgentId());
						if(requestedSellers.containsAll(activeSellers)) {
							b.setActive(false, this.getMarketId());
						}
					}
				}
