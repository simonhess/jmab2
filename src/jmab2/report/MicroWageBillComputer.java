 ...
 */
public class MicroWageBillComputer extends AbstractMicroComputer implements
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:pop.getAgents()){
			LaborDemander employer = (LaborDemander) i;
			if (!employer.isDead()){
			result.put(employer.getAgentId(), employer.getWageBill());
			}
			else{
				result.put(employer.getAgentId(), Double.NaN);
			}
		}
		return result;
	}
