 ...
 *
 */
public class MicroEmployerComputer extends AbstractMicroComputer implements
	...
	public Map<Long, Double> computeVariables(MacroSimulation sim) {
		...
		for (Agent i:pop.getAgents()){
			AbstractHousehold agent=(AbstractHousehold) i;
			if(agent.isEmployed())
				result.put(agent.getAgentId(), (double)agent.getEmployer().getAgentId());
			else
				result.put(agent.getAgentId(), (double)-1);
		}
		return result;
	}
}
