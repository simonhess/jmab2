 ...
 *
 */
public class AugmentedAdaptiveExpectation implements Expectation {
	 ...
	 * 
	 */
	public AugmentedAdaptiveExpectation() {
	}
	...
	public void updateExpectation() {
		double result=adaptiveParam*this.passedValues[0][0]+(1-adaptiveParam)*passedValues [0][1];
		double errorCorrection=0;
		for(int i=0; i<nbPeriod-1;i++){
			for(int j=0;j<nbVariables;j++){
				errorCorrection+=passedValues[i][j]-passedValues[i+1][j];
			}
		}
		errorCorrection=errorCorrection/nbPeriod;
		this.expectation=result+errorCorrection;
	}
	...
	public void addObservation(double[] observation) {
		...
		this.passedValues[0][nbVariables+1]=this.expectation;
	}
	...
	public byte[] getByteArray() {
		byte[] result = new byte[24+16*(nbPeriod+(nbVariables+1))];
